...
`close` unmounts the tooltip, so the dismissal sticks — a merely hidden tooltip would reopen on the controller's next hover. It also returns focus to the controller and emits `dismiss`, which is where you persist the choice if it has to survive a reload.
