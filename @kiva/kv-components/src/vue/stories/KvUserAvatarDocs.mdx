...
With `useIconFallback`, display types 2 and 3 collapse into a single avatar icon: any user without a custom image gets the same `mdiAccountCircle` glyph on a `tw-bg-brand-100` tint, and a stored default avatar image never renders. A real custom image is unaffected. Anonymous users get the icon too, so an anonymous lender's stored image stays hidden.

The prop is opt-in — existing consumers keep the lettered and Kiva-K circles. `KvWwwHeaderBasic`'s link bar sets it.

<Story of={KvUserAvatarStories.IconFallback} />
...
- **Avatar Icon**: The `mdiAccountCircle` glyph shown in place of the letter and the Kiva logo under `useIconFallback`
