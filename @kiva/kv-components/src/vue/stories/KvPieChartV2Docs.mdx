...
- **Animation Timing**: `initialDelay` postpones the entrance animation; `growDuration` sets how fast each segment draws — and the stagger and count-up, kept in sync; `animateOut` reverses it
...
- **Per-segment grow** (`growDuration`, default `500ms`): Each segment expands clockwise from its starting angle
- **Stagger** (`growDuration`): Each segment begins as the previous one completes, so the sweep stays continuous

Count-up animations on legend pill values run in parallel with their corresponding segment animations, paced by the same `growDuration`, using cubic ease-in-out. Because one value drives the draw, the stagger, and the count-up together, lowering `growDuration` speeds up the entire sweep in sync — useful in compact, fast-moving contexts. Leaving it unset preserves the original `500ms` timing.

<Story of={KvPieChartV2Stories.DrawSpeed} />
