...
<Meta title="Forms/KvTextLink" component={KvTextLinkStories.Component} />
...
- **Link Text**: The visible text content provided via the default slot, styled with `tw-text-label` typography and link color
...
The component uses the `tw-text-label` class for consistent typography across all variants:

- 14px, medium weight, 18px line height
- Color is set to the link color (`tw-text-link`)
- Disabled state reduces opacity (`tw-opacity-low`)

Links render in sentence case. For a surface that still wants the uppercase treatment, add `tw-uppercase` at the call site:

```vue
<kv-text-link href="/about" class="tw-uppercase">Learn more</kv-text-link>
```

`tw-text-label` sets no `text-transform`, so nothing competes with that class and its position in the stylesheet does not matter.

<Story of={KvTextLinkStories.CaseTreatments} />
