import { lenderRepaymentTermValueMap, EIGHT_MONTHS_KEY } from '../lenderRepaymentTerms';
import {
	getUpdatedNumLoansFundraising,
	getUpdatedRegionsNumLoansFundraising,
	getCheckboxLabel,
	transformRadioGroupOptions,
	getFilterKeyFromValue,
	getDisplayedNumber,
} from '../filterOptionUtils';
import {
...
describe('filterOptionUtils.ts', () => {
	...
	describe('getUpdatedRegionsNumLoansFundraising', () => {
		it('should handle undefined and empty', () => {
			expect(getUpdatedRegionsNumLoansFundraising(undefined, [])).toEqual([]);
			expect(getUpdatedRegionsNumLoansFundraising([], [])).toEqual([]);
		});
		...
		it('should update numLoansFundraising', () => {
			...
			expect(getUpdatedRegionsNumLoansFundraising(mockTransformedRegions, [nextSouthAmerica])).toEqual([
		...
		it('should add missing regions', () => {
			...
			expect(getUpdatedRegionsNumLoansFundraising([southAmerica], [middleEast])).toEqual([middleEast, updatedSouthAmerica]);
		});
	});
	...
	describe('getDisplayedNumber', () => {
		...
		it('should return value with precision based on step', () => {
			...
			expect(getDisplayedNumber(4.001, false, undefined, 0.001)).toBe('4.001');
			expect(getDisplayedNumber(4.0011, false, undefined, 0.001)).toBe('4.001');
			expect(getDisplayedNumber(4.0016, false, undefined, 0.001)).toBe('4.002');
		});
	});
});
