import partners, { transformPartners } from '../partners';
...
describe('partners.ts', () => {
	describe('default', () => {
		...
		describe('getValidatedSearchState', () => {
			it('should handle undefined', () => {
				const result = partners.getValidatedSearchState({ partnerId: [1] }, undefined);

				expect(result).toEqual({ partnerId: [] });
			});

			it('should handle empty', () => {
				const result = partners.getValidatedSearchState({}, mockAllFacets);

				expect(result).toEqual({ partnerId: [] });
			});

			it('should validate partner IDs', () => {
				let result = partners.getValidatedSearchState({ partnerId: [-1] }, mockAllFacets);

				expect(result).toEqual({ partnerId: [] });

				result = partners.getValidatedSearchState({ partnerId: undefined }, mockAllFacets);
		...
		describe('getFilterFromQuery', () => {
			it('it should handle undefined facets', () => {
				...
				const result = partners.getFilterFromQuery(query, undefined);
			...
			it('it should get filter', () => {
				...
				const result = partners.getFilterFromQuery(query, mockAllFacets);
		...
		describe('getQueryFromFilter', () => {
			it('should push partner IDs', () => {
				...
				const result = partners.getQueryFromFilter(state);
