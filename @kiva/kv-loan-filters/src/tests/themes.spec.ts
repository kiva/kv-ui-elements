import themes, { transformThemes } from '../themes';
...
describe('themes.ts', () => {
	describe('default', () => {
		...
		describe('getValidatedSearchState', () => {
			it('should handle undefined', () => {
				const result = themes.getValidatedSearchState({ themeId: [1] }, undefined);

				expect(result).toEqual({ themeId: [] });
			});

			it('should handle empty', () => {
				const result = themes.getValidatedSearchState({}, mockAllFacets);
			...
			it('should validate theme', () => {
				...
				const result = themes.getValidatedSearchState(state, mockAllFacets);
		...
		describe('getFilterFromQuery', () => {
			it('it should handle undefined facets', () => {
				...
				const result = themes.getFilterFromQuery(query, undefined);
			...
			it('it should get filter', () => {
				...
				const result = themes.getFilterFromQuery(query, mockAllFacets);
		...
		describe('getQueryFromFilter', () => {
			it('should push theme IDs', () => {
				...
				const result = themes.getQueryFromFilter(state);
	...
	describe('transformThemes', () => {
		...
		it('should filter, transform, and sort', () => {
			...
			const result = transformThemes(mockFilteredThemes, mockThemes as never);
