import partnerAvgProfitability, { MIN, MAX } from '../partnerAvgProfitability';
import { createMinMaxRange } from '../minMaxRangeUtils';

describe('partnerAvgProfitability.ts', () => {
	describe('default', () => {
		describe('getFilterChips', () => {
			it('should handle undefined', () => {
				expect(partnerAvgProfitability.getFilterChips({})).toEqual([]);
			});
		...
		describe('getValidatedSearchState', () => {
			it('should handle undefined', () => {
				const result = partnerAvgProfitability.getValidatedSearchState({});
			...
			it('should validate average profitability', () => {
				...
				const result = partnerAvgProfitability.getValidatedSearchState(state);
		...
		describe('getFilterFromQuery', () => {
			it('it should get filter', () => {
				...
				const result = partnerAvgProfitability.getFilterFromQuery(query);
		...
		describe('getQueryFromFilter', () => {
			it('should push average profitability', () => {
				...
				const result = partnerAvgProfitability.getQueryFromFilter(state);
		...
		describe('getFlssFilter', () => {
			...
			it('should return filters', () => {
				expect(partnerAvgProfitability.getFlssFilter({ partnerAvgProfitability: { min: 1, max: 2, __typename: 'MinMaxRange' } }))
