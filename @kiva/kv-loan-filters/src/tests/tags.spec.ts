import tags, { transformTagName, transformTags } from '../tags';
...
describe('tags.ts', () => {
	describe('default', () => {
		...
		describe('getValidatedSearchState', () => {
			it('should handle undefined', () => {
				const result = tags.getValidatedSearchState({ tagId: [1] }, undefined);

				expect(result).toEqual({ tagId: [] });
			});

			it('should handle empty', () => {
				const result = tags.getValidatedSearchState({}, mockAllFacets);
			...
			it('should validate tag', () => {
				...
				const result = tags.getValidatedSearchState(state, mockAllFacets);
		...
		describe('getFilterFromQuery', () => {
			it('it should handle undefined facets', () => {
				...
				const result = tags.getFilterFromQuery(query, undefined);
			...
			it('it should get filter', () => {
				...
				const result = tags.getFilterFromQuery(query, mockAllFacets);
		...
		describe('getQueryFromFilter', () => {
			it('should push tag IDs', () => {
				...
				const result = tags.getQueryFromFilter(state);
	...
	describe('transformTags', () => {
		...
		it('should transform and sort', () => {
			...
			const result = transformTags(flssTags, allTags as never);
