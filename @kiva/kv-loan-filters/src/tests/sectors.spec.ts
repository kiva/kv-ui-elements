import sectors, { transformSectors } from '../sectors';
...
describe('sectors.ts', () => {
	describe('default', () => {
		...
		describe('getValidatedSearchState', () => {
			it('should handle undefined', () => {
				const result = sectors.getValidatedSearchState({ sectorId: [1] }, undefined);

				expect(result).toEqual({ sectorId: [] });
			});

			it('should handle empty', () => {
				const result = sectors.getValidatedSearchState({}, mockAllFacets);
			...
			it('should validate sector ID', () => {
				...
				const result = sectors.getValidatedSearchState(state, mockAllFacets);
		...
		describe('getFilterFromQuery', () => {
			it('it should handle undefined facets', () => {
				...
				const result = sectors.getFilterFromQuery(query, undefined);
			...
			it('it should get filter', () => {
				...
				const result = sectors.getFilterFromQuery(query, mockAllFacets);
		...
		describe('getQueryFromFilter', () => {
			it('should push sector IDs', () => {
				...
				const result = sectors.getQueryFromFilter(state);
	...
	describe('transformSectors', () => {
		...
		it('should filter, transform, and sort', () => {
			...
			const result = transformSectors(mockFilteredSectors, mockAllSectors as never);
