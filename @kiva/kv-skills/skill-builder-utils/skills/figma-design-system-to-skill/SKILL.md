...
A different kind of skill also exists: the **kiva-tailwind-css** skill (in the kiva-design-to-code plugin) documents the **shipped mechanics** of the Kiva Tailwind preset and is *code-canonical* (not Figma-canonical). You don't author skills like it with this process, but every design-intent skill's "Using with Tailwind" section refers to it — treat it as the canonical reference for preset mechanics rather than re-explaining them.
...
These two sections are where an otherwise Figma-canonical skill becomes **code-canonical**. They sit where the old single "Current code state" section used to — right before "Figma source references" — and they defer to the **kiva-tailwind-css** skill (in the kiva-design-to-code plugin), the canonical shipped-mechanics guide to the Kiva preset. Don't regenerate its content here; refer to it by name.
...
1. **Hybrid setup line.** State that the utilities come from the `@kiva/kv-tokens` preset, point to the **kiva-tailwind-css** skill → "Consuming the preset" for projects not yet wired up, and point to the "Without the preset" subsection for projects that won't adopt it.
